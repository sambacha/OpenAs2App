...
public class HTTPUtil {
	...
	public static byte[] readHTTP(InputStream inStream, OutputStream outStream, InternetHeaders headerCache,
			List<String> httpRequest) throws IOException, MessagingException {
		byte[] data = null;
		...
		if (logger.isTraceEnabled())
			logger.trace("HTTP received request: " + request[0] + "  " + request[1] + "\n\tHeaders: "
					+ printHeaders(headerCache.getAllHeaders(), "==", ";;"));
	...
	public static String[] readRequest(InputStream in) throws IOException {
		...
		if (tokenCount >= 3) {
			...
			throw new IOException("Invalid HTTP Request: Token Count - " + tokenCount + "::: String length - "
					+ strBuf.length() + " ::: String - " + strBuf.toString());
		}
	}
	...
	public static ResponseWrapper execRequest(String method, String url
	    ...
	    if (needSsl(urlObj)) {
	        /*
	         * Must be registered via connection manager (see below),
	         * registration via httpBuilder.setSSLSocketFactory is ignored when a connection manager is set.
	         */
	        sslCsf = buildSslFactory(urlObj, options);
	    }

	    RequestBuilder rb = getRequestBuilder(method, urlObj, params, headers);
	    rb.setConfig(buildRequestConfig(options));
	...
	private static boolean needSsl(URL urlObj) {
	    return urlObj.getProtocol().equalsIgnoreCase("https");
	}
    ...
    private static RequestBuilder getRequestBuilder(String method, URL urlObj
        ...
        else throw new IllegalArgumentException("Illegal HTTP Method: " + method);
    ...
    private static RequestConfig buildRequestConfig(Map<String, String> options) {
	 ...
	 */
	public static void sendHTTPResponse(OutputStream out, int responseCode, ByteArrayOutputStream data,
			Enumeration<String> headers) throws IOException {
		...
		if (data == null || data.size() < 1) {
			...
			boolean sendHttpCodeAsString = "true"
					.equals(Properties.getProperty("send_http_code_as_string_when_no_data", "true"));
	 ...
	 */
	public static HttpURLConnection getConnection(String url, boolean output, boolean input, boolean useCaches,
			String requestMethod) throws OpenAS2Exception {
		if (url == null)
			...
			if (urlObj.getProtocol().equalsIgnoreCase("https")) {
				...
				if (selfSignedCN != null) {
					...
					try {
						...
						throw new OpenAS2Exception(
								"Self-signed certificate URL connection failed connecting to : " + url, e);
					}
				}
				conn = connS;
			} else {
